function Output = HybridTrackNO(originalImageKev,varargin)
...
try
    ...
    if strcmpi(err.identifier,'HT:infLoop')
        Output.img = preparedImageKev;
        Output.Etot = sum(preparedImageKev(:));
...
if Options.PlotStyle.multiple(13)
    ...
    [~, Options.PlotStyle.axesPosition] = ...
        HtPlotImage(preparedImage, Options.PlotStyle.cmap, ...
        Options.PlotStyle.axesPosition, ...
...
%%if index is wrong, can uncomment this to manually fix
%tmp=input('chosenIndex?\n');
% if ~isempty(tmp);
EdgeSegments.chosenIndex=~(EdgeSegments.chosenIndex-1)+1;
% end
...
while ~isFinished
    ...
    previousPoints = 1 : size(Ridge.positionPix,1)-1;
    dx = thisXPix - Ridge.positionPix(previousPoints,1);
    dy = thisYPix - Ridge.positionPix(previousPoints,2);
...
if isInInfiniteLoop
    error('HT:infLoop','Infinite loop. Need to handle this somehow')
end
% trim last point
...
function Ridge = HtTakeOneStep(preparedImage, Ridge, Options, ...
...
for cutAngleNo = 1:length(theseCutAnglesIndices)
    ...
    minX = 1;
    minY = 1;
    maxX = size(preparedImage,1);
    maxY = size(preparedImage,2);
    ...
    thisCutEnergyKev = interp2(preparedImage, thisCutYPix, thisCutXPix, ...
        Options.cutInterpolationMethod);
...
function Output = HtSetOutput(preparedImage, Options, EdgeSegments, Ridge, ...
    Measurement)
% function Output = HtSetOutput(preparedImage, Options, EdgeSegments, Ridge, ...
%     Measurement)

Output.img = preparedImage;
Output.Etot = sum(preparedImage(:));
