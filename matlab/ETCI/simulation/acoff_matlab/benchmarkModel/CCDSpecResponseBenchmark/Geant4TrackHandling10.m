 function currentOut = Geant4TrackHandling10(varargin)
...
%
% Modified 9/12/2012. Adding New Mode of Handing Geant4 Tracking Data to
% DiffuseTrack6.m  : 
...
%
% Modified 11/2013. A. Coffer. Set-up to use the New Tracking Matrix 2013.
%
% Depends on:
...
%       12: Step length                     (mm)
%       13: Initial energy (before step)    (eV) (obsolet Geant4 Value)(E-DE)
%       14: Final energy (after step)       (eV)
%       15: Delta Energy (Along step)(neg)  (eV) (obsolet Geant4 Value)
%       16: Total Energy Deposited (Edep)   (eV)
%       17: Layer ID                        (int)
...
%  noise: specify the sigma of the black level noise.
...
% New in Geant4TrackHandling10:
% RadDecayFlag: (Photon source info looks different for Tracking Matrix)
%   Default: false
%   If set to true: 
...
while true
    ...
    elseif (strcmpi(varargin{i}, 'depthcoordinates') || ...
        ...
        if nargin>i && isnumeric(varargin{i+1}) && length(varargin{i+1})==2
            ...
            message = [ '... Using default depthcoordinates of: ' num2str(depthCoordinates)];
            warning(message);
            i=i+2;
        end
    ...
    elseif strcmpi(varargin{i}, 'bypassPhotonErrorFlag')
        ...
        %i=i+2;
    elseif strcmpi(varargin{i}, 'RadDecayFlag') || ...
           strcmpi(varargin{i}, 'RadDecay') 
        if nargin>i && (isnumeric(varargin{i+1}) || islogical(varargin{i+1}))
            radDecayFlag = logical(varargin{i+1});
            i=i+2;
        else
            error('problem with bypassPhotonErrorFlag argument')
        end
...
%       17: Layer ID                        (int)
    ...
    inddE = 15;     % New TM ... Not the same as old ingdE... But the same for photons...
    indEdep = 16;   % New to use... instead of (FinalE-InitE) or dE...
    ingLayerID = 17;
...
if strcmp(inputMode,'coinc') || strcmp(inputMode,'single')
    ...
    if radDecayFlag
        ...
        % 
        findNewSourcePhoton = find(g4Matrix(:,indCharge) == 0);
    ...
    for sourcePhotonIndex = 1:length(findNewSourcePhoton)-1
        ...
        if oldCodeFlag
            ...
            if ~multiNoise && isempty(noise)
                ...
                if length(findNewSourcePhoton)==2 %2 Added space 
        ...
        if radDecayFlag
            % Each Photon Step Recorded post GetCoincidentETrackEvents9.m
            % is a source Photon
            % NOT YET WORKING PROPERLY   >.<
            sourcePhotonID = g4MatrixCurrent(find(g4MatrixCurrent(:,indCharge)==0,indTrackID),1); %findNewSourcePhoton);
...
%             tic;
            ...
            %currentCheat.Etot = 0;     %so we can  save the total initial energy of a primary electron.
...
%             disp(['xray distance is ',num2str(minDistance),' mm'])
            ...
            if minDistance > 1e-3   %1 um...
                ...
                currentCheat.Exray = g4MatrixCurrent(findXRay(primaryElectronIndex),indEdep) * 1e-3;  %eV to keV
    ...
    %                 cheat.sourcePhotonScatterAngle = 
                ...
                if g4MatrixCurrent(findXRay(primaryElectronIndex),indFinalPos(indDepth)) > min(depthCoordinates) && ...      zmin
                        ...
                        g4MatrixCurrent(findXRay(primaryElectronIndex),indEdep) > 0;     %nonzero deposition
...
%             tic;
            ...
            for bremsIndex = 1:length(bremsList)
                ...
                lgCCDactiveVolume = g4MatrixCurrent(:,indTrackID) == bremsList(bremsIndex) & ...       current particle
                    ...
                    g4MatrixCurrent(:,indFinalPos(indLateral(2))) > imageBoundsMM(2,1) & ...    ymin
                    g4MatrixCurrent(:,indFinalPos(indLateral(2))) < imageBoundsMM(2,2) & ...    ymax
                    g4MatrixCurrent(:,indEdep) > 0;                                   % deposited energy
...
%             disp(['Diffusing all brems took ',num2str(toc),' seconds'])
        ...
        if ~multiNoise && ~multiPixelSize
            ...
            segmentThresholdCurrent = segmentThresholdBase + segmentThresholdSlope * pixelSize(1)^2;
            ...
            for pixelSizeIndex = 1:length(pixelSize)
                if isempty(noise)
                    ...
                    segmentThresholdCurrent = segmentThresholdBase + ...
                        segmentThresholdSlope * pixelSize(pixelSizeIndex)^2;
                    ...
                    %   pixelSize is saved in the DiffuseTrack8 output, 
                ...
                for noiseIndex = 1:length(noise)
                    ...
                    if noise(noiseIndex)==0
                        ...
                        %noise: adjust base threshold for noise, then adjust for pixel size
                        segmentThresholdCurrentBase = segmentThresholdBase / 0.01874 * noise(noiseIndex);
...
function [imageFinal, currentCheat] = DiffuseAndAdd(psft, g4MatrixSegment, ...
    ...
    %diffuse
    D = DiffuseTrack8(psft, 'geant4', g4MatrixSegment, ...
