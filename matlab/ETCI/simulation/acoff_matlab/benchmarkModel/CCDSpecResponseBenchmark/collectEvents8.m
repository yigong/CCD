function out = collectEvents8(filenamePhrase,E, plotFlag)
...
lrcFlag = false; % PlotFlag == 1 (for plotting and Windowed running of matlab)

% Add Matlab Function to Path
if lrcFlag
    addpath /global/home/users/acoffer/acoff/ETCI/simulation/acoff_matlab/benchmarkModel/CCDSpecResponseBenchmark;
else
...
Event = {};
out = {};
primaryPhotons = (1e6); % Number of primary photons for each file
numPrimaryDataFiles = 20;
badCCD = 0; 
...
for i=1:length(fnames)
    ...
    try
        ...
        if exist('Event','var')
            res = tempCollectCCDEnergies8(Event); % Saves calculated events
            k = k+1;
        else
    ...
    try      
        ...
        error3{1,i} =  res.error3;
        error4{1,i} = res.error4;
...
if nps<1
    ...
    nps = primaryPhotons.*numPrimaryDataFiles;
end
...
save(sname2, 'EimgTot','Edep', 'E0','Eesc', 'Emulti', 'multi', 'EimgContained','EimgNonContained','nps'); % ...
            %'multi');
