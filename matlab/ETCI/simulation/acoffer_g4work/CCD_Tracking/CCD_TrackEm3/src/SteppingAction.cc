...
void SteppingAction::WriteToOutput(G4SteppingManager* pSM)
{
  ...
  if ((absorberHit == 1) && (Edep > 0) && (Chg ==-1))	fEventAct->ChangeAbsorberHitFlag(); // 2/14/14
    ...
    
  {
    ...
    VerboseStepInformationStream    << ID << "," << PID << "," << stepNum << "," << Chg << ","
    << setprecision (prec)
    << posA.getX()/mm << ","
    << posA.getY()/mm << ","
    << posA.getZ()/mm << ","
    ...
    << E/eV << ","      // Final Energy (good)
    //<< DE/eV << ","     // Delta Energy (obsolet in next Geant4 version)
    << Edep/eV <<  ","  // Total Energy Deposited (Geant4) On current Step
    << AvgStepEnergy/eV << "," // Sum of 'numberESteps' Steps Geant4 "Total Energy Deposited"
    << Idnow << G4endl;
