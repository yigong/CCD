...
if LRC
    ...
    loadpre = 'Mat_662KeV_pBeam_1000nm_10k_05';
    loadsuf = '.dat';
...
%for j = 1:length(foldernum)     %each folder... (as if we'll ever finish the first one)
    ...
    parfor i = 1:length(flist)
        try
            %loadDataPath
            %cd loadDataPath;
            G4THworker2(loadDataPath,flist,savepath,savepre,loadpre,loadsuf,savesuf,noise,pixelSize,pixelNoiseMatrix,i,placeholdersuf,psft, depthCordinates);
        catch err
