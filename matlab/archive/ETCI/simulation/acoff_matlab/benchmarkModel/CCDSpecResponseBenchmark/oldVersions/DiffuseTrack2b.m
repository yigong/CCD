...
for i=1:N
    fcur = find(M(:,1)==i,1,'first');  %first record of this particle
    if isempty(fcur)
        %this particle never got written: ignore it.
        f1(i) = nan;
        q(i) = nan;
        Ep(i) = nan;
        p(i) = nan;
    else
        f1(i) = fcur;               %first record of this particle
