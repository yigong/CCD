function res = collectCCDEnergies8(Event)
...
% Save Image Energy, if there; is no energy, it gets skipped. 
    try
        if  ~isempty(Event{1,i}.E)
            %for j = 1:length(Event{1,i}.out.E)
                
                 res.EimgTot(1,n) = Event{1,i}.cheat.Etot(:);

                 % Sort Image Energies Into Contained and Non-Contained
                 %if exist('Event{1,i}.out.E','var') DT{1, 10}.cheat.E0
                 if ( (Event{1,i}.cheat.E0-Event{1,i}.T.E) > 0)
                     res.EimgNonContained(1,n) = Event{1,i}.T{1,1}.E;
                     %res.E_G4diff_nonContatined_Image(1,n)= (Event{1,i}.out.cheat.Etot - Event{1,i}.out.E(1));
                 else
                     res.EimgContained(1,n) = Event{1,i}.T{1,1}.E;
