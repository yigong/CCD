function [Event] = RunTracks8(Res,minEThresh, dEdx_ref, psft, depthCordinates)
...
% UPDATED TWO: 11/27/2013 - Coffer - Modifying for the New Tracking Matrix
% load(filename);
% 
% OUTPUTS:
...
for i = 1:length(Res)
  ...
  %  A.err = 'No Attempt';
     ...
     %   try
            ...
            out = Geant4TrackHandling8(R2,'coinc','psft',psft,'CCDsize',[2000,2000],'bypassPhotonErrorFlag', 1, 'driftdim', 'x', 'depthcoordinates', depthCordinates);
...
%         'DiffIMG', D, 'depthCordinates', depthCordinates);  %'geDetector',GeDetector,'armInfo', armInfo, ...'boolenQuant',[QET,QGT]);
    ...
    Event{i} = struct('out',out, 'depthCordinates', depthCordinates);    
    % Reset Variables
    En = []; RDF = []; RHT = [];  Egam = [];
