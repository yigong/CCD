...
{
  // World and Cryostat Cut Values
  fCurrentDefaultCut   = 2.0*cm;
  fCutForGamma         = 2.0*mm; //fCurrentDefaultCut;
  fCutForElectron      = fCurrentDefaultCut;
  fCutForPositron      = fCurrentDefaultCut;
  // CCD 'Calor' Cut Values
  fProductionCutForCalor = 100.0*nm; // Producion Cut for CCD
  //regName = "Layer-A";
  fNbOfAbsor = 3;
  absorName[0] = "Layer-A";
  absorName[1] = "Layer-B";
  absorName[2] = "Layer-C";
  fRangeCutForCalor = 100.00*nm; // Range Cut for Region
...
#include "G4Decay.hh"
// Adding Radioactive Decay to ConstructProcess...  
...
{      
  ...
  //SetCutsWithDefault();  // --> Can be 'Turned on to Set Cuts Back to defaults (1mm)
  
  //Production Cuts
  for (G4int j=1; j<=fNbOfAbsor; j++){
    G4Region* region = G4RegionStore::GetInstance()->GetRegion(absorName[j]);
    G4ProductionCuts* cuts = new G4ProductionCuts;
    cuts->SetProductionCut(fProductionCutForCalor);
    region->SetProductionCuts(cuts);
    //Particle Cuts
    //SetParticleCuts
    SetCutsForRegion(fRangeCutForCalor,absorName[j]);
  }
