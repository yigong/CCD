...
int main(int argc,char** argv)
{
  ...
  // Use time in milliseconds to set the seeds for rand
  int t_ms = QDateTime::currentMSecsSinceEpoch();
  srand(t_ms);
  long seeds[] = {rand(), rand()};
  G4cout << t_ms << " " << seeds[0] << " " << seeds[1];
...
//  G4ProductionCutsTable::GetProductionCutsTable()->SetEnergyRange(250*eV, 1*GeV);
  B1ModularPhysics* physicsList = new B1ModularPhysics;
