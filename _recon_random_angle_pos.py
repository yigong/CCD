...
def main():
    ...
    LLSEout = np.array(pool.map(partial_func, f_files))
    print LLSEout
    # 1/0
    LLSE = dict()
    LLSE['alphaT'] = LLSEout[:, 0]
    LLSE['alphaM'] = LLSEout[:, 1]
    pickle.dump(LLSE, open('%s/alpha_px.p' %(recon_dir), 'wb'))
    LLSEplot = LLSEout[:, 0] - LLSEout[:, 1] # true - measure
    # # load mat
    ...
    #plot
    binW = 1 # deg
    binCenters = np.arange(-30, 30.1, binW)
    binEdges = np.arange(binCenters[0]-0.5*binW, binCenters[-1]+0.51*binW, binW)
    LLSE_counts, _ = np.histogram(LLSEplot, binEdges)
    ...
    # print figure
    canvas.print_figure('%s/LLSE.png' %(recon_dir))
