...
def ridge_follow(fits_file, out_dir='', plot_flag=False, plot_save_dir=None):
    try:
    # if True:
        # initialization
        ...
        if plot_flag:
            ...
            print alpha_linearReg, alpha_median
            title('threshold = %s || linear_reg: %.1f || median: %s  ' %(threshold, alpha_linearReg, alpha_median))
            ...
            ylabel('derivative of Alpha (deg)', color='red')
            ylim([-30, 80])
            for label in gca().get_yticklabels():
                label.set_color('red')
        ...
        # output
        if out_dir:
            pickle.dump(result, open('%s/%s.p' %(out_dir, file_idx), 'wb'))
        return result

    except: 
   #  else:
        tb = traceback.format_exc()
        open('%s/%s.err' %(out_dir, file_idx), 'w').write(tb)
        return 0
